    ...
    try {
      ...
      if (progressData.status === 'completed') {
        if (progressData.sent > 0 && progressData.failed === 0) {
          // All succeeded
          addSuccessToast(
            t('Newsletter sent successfully to all %s recipients!', progressData.sent)
          );
        } else if (progressData.sent > 0 && progressData.failed > 0) {
          // Partial success
          addSuccessToast(
            t('Newsletter partially sent. Success: %s, Failed: %s', progressData.sent, progressData.failed)
          );
        } else if (progressData.sent === 0 && progressData.failed > 0) {
          // All failed
          addDangerToast(
            t('Newsletter sending failed for all %s recipients. Check SMTP configuration.', progressData.failed)
          );
        }
  ...
  return (
    <StyledContainer>
      ...
      <div className="form-section">
        <Typography.Title level={4} style={{ marginBottom: theme.sizeUnit * 4 }}>
          {t('Compose Newsletter')}
        </Typography.Title>
        <Typography.Paragraph type="secondary" style={{ marginBottom: theme.sizeUnit * 6 }}>
        ...
        <div className="form-row">
          ...
          >
            {t('Write your newsletter content in HTML format. Available variables: {user_first_name}, {user_last_name}, {user_username}, {user_email}, {unsubscribe_link}')}
          </Typography.Text>
          <Input.TextArea
      ...
      <Modal
        ...
        footer={[
          <Button 
            key="close" 
            onClick={() => setSendingProgress(prev => ({ ...prev, visible: false }))}
      ...
      >
        <Space direction="vertical" style={{ width: '100%' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <Typography.Text type={sendingProgress.failed > 0 ? 'danger' : 'secondary'}>
          ...
          {sendingProgress.failedEmails.length > 0 && (
            <div>
              <Typography.Text strong>{t('Failed recipients:')}</Typography.Text>
          ...
          {sendingProgress.status === 'completed' && (
            <Alert
              type={sendingProgress.sent > 0 && sendingProgress.failed === 0 ? "success" : 
                    sendingProgress.sent > 0 && sendingProgress.failed > 0 ? "warning" : "error"}
              message={
